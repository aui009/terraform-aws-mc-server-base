...
PATTERN_VILLAGER_MESSAGE="Villager"
...
do
        ...
        if ! echo "$LINE" | grep -qE "$PLAYER_MESSAGE_PATTERN" && ! echo "$LINE" | grep -qE "$PATTERN_VILLAGER_MESSAGE"; then
           echo "$(date +'%Y-%m-%d %H:%M:%S'): Line: $LINE does not contain player message or villager message, checking for other patterns"
           if echo "$LINE" | grep -q "$PATTERN_JOINED";then
           ...
           elif echo "$LINE" | grep -q "$PATTERN_LEFT";then
               player_name=$(echo "$LINE" | awk '{print $4}')
          ...
          elif echo "$LINE" | grep -q "$PATTERN_ACHIEVEMENT";then
               player_name=$(echo "$LINE" | awk '{print $4}')
           ...
           echo "$(date +'%Y-%m-%d %H:%M:%S'): No Matches for line: $LINE"
        fi
